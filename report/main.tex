...
(Section~\ref{sec:exp-extraction}). Taken together, these results show that a general-purpose VLM, driven only by prompt engineering, can convert the large majority of social-housing plans into well-formed topological graphs. The manual spot-check (Section~\ref{sec:exp-spotcheck}) confirms that these graphs are reliably \emph{well-formed}, even where exact node counts are not always perfect---the residual semantic gap being quantified there and discussed among the limitations below.
