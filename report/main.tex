...
\paragraph{End-to-end success rate.} Re-expressing this cascade as a quality
rate over the plans the system is \emph{meant} to handle---the 665 true
aggregations---the two automated gates compose multiplicatively: a 93.8\,\%
classification recall (Section~\ref{sec:exp-classification}) followed by a
90.2\,\% structural pass rate (Section~\ref{sec:exp-extraction}) yields an
estimated \textbf{84.6\,\%} ($0.938 \times 0.902$) of true aggregation plans
turned into a well-formed graph. This is a \emph{conservative} estimate: the
90.2\,\% is measured over all 691 predicted-aggregation graphs and is therefore
diluted by misclassified non-aggregations that fail validation by construction,
so the true aggregations alone plausibly pass at a higher rate.

\paragraph{Structural versus semantic success.} The 84.6\,\% figure measures
\emph{structural} correctness: the graph is well-formed and passes every hard
check. It does not guarantee that every apartment was counted exactly, which is
the stricter bar of the manual spot-check (Section~\ref{sec:exp-spotcheck}),
where only a minority of graphs were judged a perfect node-for-node match. The
two numbers are complementary: the pipeline reliably produces \emph{usable,
well-formed} graphs for the large majority of the plans it is meant to handle,
while exact node-level fidelity on dense or legend-driven layouts remains the
principal open challenge and the main target of the human-in-the-loop stage.

Crucially, the structural rate (84.6\,\%) and the semantic rate are measured at
\emph{different stages, on different sets of graphs}. The automated validation
(Step~4) runs first over all extracted graphs and gives the structural figure;
the semantic spot-check (Section~\ref{sec:exp-spotcheck}) runs afterwards on a
random sample of 30 graphs drawn only from those that \emph{passed} that
validation. The two are therefore not competing estimates of the same quantity
but successive, stricter filters. Combining them gives the realistic end-to-end
picture: of every 100 true aggregation plans entering the pipeline, roughly 85
yield a structurally valid graph, and of those, the spot-check suggests only
about one in nine is additionally a flawless node-for-node match---so on the
order of \textbf{10\,\%} are correct in the strict semantic sense, while the
large remainder are structurally sound graphs needing only minor node-count
corrections. This strict figure is a lower bound, set by a deliberately
unforgiving criterion in which a single miscounted apartment fails an entire
graph.

\paragraph{Successful end-to-end extractions.} To balance the error-focused
figures of the previous sections, Figure~\ref{fig:successes} shows three plans
that traverse the entire pipeline without error: each is correctly classified
as an aggregation, every apartment and circulation core is recovered, and the
resulting graph is both structurally valid and a faithful node-for-node match
to the drawing. These are the spot-check plans judged \emph{fully correct}
